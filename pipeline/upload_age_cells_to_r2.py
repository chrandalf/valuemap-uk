#!/usr/bin/env python3
"""
Upload age_cells_<grid>.json.gz artifacts to Cloudflare R2.
...
Run build_age_cells.py first to generate these files.
"""
...
from paths import MODEL_CENSUS_DIR, R2_ARCHIVE_DIR
...
def backup_remote_objects(
    ...
    archive_path = backup_dir / f"r2_backup_{timestamp}_{prefix_label}_age.zip"
    ...
    with ZipFile(archive_path, "a", compression=ZIP_DEFLATED) as zf:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Upload age_cells_<grid>.json.gz artifacts to Cloudflare R2"
    )
    parser.add_argument(
        "--data-dir",
        default=str(MODEL_CENSUS_DIR),
        help="Directory containing age_cells_*.json.gz",
    )
    parser.add_argument("--prefix", default=None, help="Optional R2 object prefix (overrides R2_PREFIX)")
    parser.add_argument(
        "--no-backup-before-upload",
        action="store_true",
        help="Skip pre-upload backup of current remote R2 objects",
    )
    parser.add_argument(
        "--backup-dir",
    ...
    files = [
        data_dir / "age_cells_1mile.json.gz",
        data_dir / "age_cells_5km.json.gz",
        data_dir / "age_cells_10km.json.gz",
        data_dir / "age_cells_25km.json.gz",
    ]
