...
Run build_commute_cells.py first to generate these files.
"""
...
def backup_remote_objects(
    ...
    archive_path = backup_dir / f"r2_backup_{timestamp}_{prefix_label}_commute.zip"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Upload commute_cells_<grid>.json.gz artifacts to Cloudflare R2"
    )
    parser.add_argument(
        "--data-dir",
        default=str(MODEL_CENSUS_DIR),
        help="Directory containing commute_cells_*.json.gz",
    )
