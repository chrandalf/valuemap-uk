...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolveSchoolObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? env.SCHOOL_OVERLAY_KEY ?? "school_overlay_points.geojson.gz");
      return Response.json({ error: `School overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-School-Key", resolved.key);
    ...
    return new Response(JSON.stringify({ error: "School overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-School-Key", resolved.key);
    headers.set("X-School-Size", String(resolved.object.size ?? 0));
    if (wantsPlain) headers.set("X-School-Plain", "1");
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "school_overlay_points.geojson.gz";
}

async function resolveSchoolObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? env.SCHOOL_OVERLAY_KEY ?? "school_overlay_points.geojson.gz");
