...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolvePharmacyObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "pharmacy_overlay_points.geojson.gz");
      return Response.json({ error: `Pharmacy overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-Pharmacy-Key", resolved.key);
    ...
    if (wantsPlain && resolved.key.endsWith(".gz") && resolved.object.body) {
      ...
      headers.set("X-Pharmacy-Plain", "1");
    ...
    return new Response(JSON.stringify({ error: "Pharmacy overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-Pharmacy-Key", resolved.key);
    headers.set("X-Pharmacy-Size", String(resolved.object.size ?? 0));
    if (wantsPlain) headers.set("X-Pharmacy-Plain", "1");
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "pharmacy_overlay_points.geojson.gz";
}

async function resolvePharmacyObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(
    url.searchParams.get("key") ?? "pharmacy_overlay_points.geojson.gz"
  );
