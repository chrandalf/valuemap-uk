...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolveBusStopObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "bus_stop_overlay_points.geojson.gz");
      return Response.json({ error: `Bus stop overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-BusStop-Key", resolved.key);
    ...
    return new Response(JSON.stringify({ error: "Bus stop overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-BusStop-Key", resolved.key);
    headers.set("X-BusStop-Size", String(resolved.object.size ?? 0));
    if (wantsPlain) headers.set("X-BusStop-Plain", "1");
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "bus_stop_overlay_points.geojson.gz";
}

async function resolveBusStopObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(
    url.searchParams.get("key") ?? "bus_stop_overlay_points.geojson.gz"
  );
