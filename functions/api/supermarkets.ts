...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolveSupermarketObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "supermarket_overlay_points.geojson.gz");
      return Response.json({ error: `Supermarket overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-Supermarket-Key", resolved.key);
    ...
    if (wantsPlain && resolved.key.endsWith(".gz") && resolved.object.body) {
      ...
      headers.set("X-Supermarket-Plain", "1");
    ...
    return new Response(JSON.stringify({ error: "Supermarket overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const url = new URL(request.url);
    const wantsPlain = url.searchParams.get("plain") === "1";
    if (resolved.key.endsWith(".gz") && !wantsPlain) headers.set("Content-Encoding", "gzip");
    headers.set("X-Supermarket-Key", resolved.key);
    headers.set("X-Supermarket-Size", String(resolved.object.size ?? 0));
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "supermarket_overlay_points.geojson.gz";
}

async function resolveSupermarketObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "supermarket_overlay_points.geojson.gz");
  ...
  for (const candidate of candidates) {
    ...
    if (attempt) return { object: attempt, key: candidate };
  }
