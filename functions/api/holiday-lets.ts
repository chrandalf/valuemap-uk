...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolveHolidayLetObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "holiday_let_overlay_points.geojson.gz");
      return Response.json({ error: `Holiday let overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-HolidayLet-Key", resolved.key);
    ...
    if (wantsPlain && resolved.key.endsWith(".gz") && resolved.object.body) {
      ...
      headers.set("X-HolidayLet-Plain", "1");
    ...
    return new Response(JSON.stringify({ error: "Holiday let overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-HolidayLet-Key", resolved.key);
    headers.set("X-HolidayLet-Size", String(resolved.object.size ?? 0));
    if (wantsPlain) headers.set("X-HolidayLet-Plain", "1");
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "holiday_let_overlay_points.geojson.gz";
}

async function resolveHolidayLetObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(
    url.searchParams.get("key") ?? "holiday_let_overlay_points.geojson.gz"
  );
