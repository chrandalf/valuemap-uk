...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolvePubObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "pub_overlay_points.geojson.gz");
      return Response.json({ error: `Pub overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-Pub-Key", resolved.key);
    ...
    return new Response(JSON.stringify({ error: "Pub overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-Pub-Key", resolved.key);
    headers.set("X-Pub-Size", String(resolved.object.size ?? 0));
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "pub_overlay_points.geojson.gz";
}

async function resolvePubObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "pub_overlay_points.geojson.gz");
