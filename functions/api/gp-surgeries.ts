...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    const resolved = await resolveGpObject(env, request);
    if (!resolved) {
      const requestedKey = normalizeRequestedKey(url.searchParams.get("key") ?? "gp_surgery_overlay_points.geojson.gz");
      return Response.json({ error: `GP surgery overlay not found for key '${requestedKey}'.` }, { status: 404 });
    }
    ...
    headers.set("X-Gp-Key", resolved.key);
    ...
    return new Response(JSON.stringify({ error: "GP surgery overlay lookup failed", message }), {
      status: 503,
...
export const onRequestHead = async ({ env, request }: { env: Env; request: Request }) => {
  try {
    ...
    headers.set("X-Gp-Key", resolved.key);
    headers.set("X-Gp-Size", String(resolved.object.size ?? 0));
...
function normalizeRequestedKey(raw: string) {
  ...
  return keyMatch ? keyMatch[0] : "gp_surgery_overlay_points.geojson.gz";
}

async function resolveGpObject(env: Env, request: Request): Promise<ResolvedObject | null> {
  const url = new URL(request.url);
  const requestedKey = normalizeRequestedKey(
    url.searchParams.get("key") ?? "gp_surgery_overlay_points.geojson.gz"
  );
