...
export type IndexPrefs = {
  ...
  gpWeight?: number;            // 0-10 importance (nearest GP surgery distance)
...
// Distance scoring for GP surgeries
const GP_GREAT_METERS         = 1_500;  // ≤1500m = full score
const GP_MAX_METERS           = 8_000;  // ≥8000m = zero
...
export default function ValueMap({
  ...
  showRgLines?: { flood?: boolean; school?: boolean; primarySchool?: boolean; station?: boolean; crime?: boolean; busStop?: boolean; pharmacy?: boolean; gp?: boolean; pub?: boolean; supermarket?: boolean };
  /**
  ...
  useEffect(() => {
    ...
    setVis(["gp-search-focus-line", "gp-search-focus-label", "gp-search-focus-ring"], cfg?.gp ?? true);
  ...
  }, [showRgLines?.flood, showRgLines?.school, showRgLines?.primarySchool, showRgLines?.station, showRgLines?.crime, showRgLines?.busStop, showRgLines?.pharmacy, showRgLines?.gp, showRgLines?.pub, showRgLines?.supermarket]);
  ...
  map.addSource("gp-overlay", {
    type: "geojson",
    data: "/api/gp-surgeries?plain=1",
    cluster: true,
    clusterMaxZoom: 10,
    clusterRadius: 40,
  });
  ...
  // ── GP surgery overlay layers (emerald green) ──
  map.addLayer({
    id: "gp-overlay-clusters",
    type: "circle",
    source: "gp-overlay",
    filter: ["has", "point_count"] as any,
    layout: {
      visibility: stateRef.current.gpOverlayMode && stateRef.current.gpOverlayMode !== "off" ? "visible" : "none",
    },
    paint: {
      "circle-color": ["step", ["get", "point_count"], "rgba(16,185,129,0.6)", 10, "rgba(5,150,105,0.75)", 50, "rgba(4,120,87,0.88)"] as any,
      "circle-radius": ["step", ["get", "point_count"], 14, 10, 19, 50, 25] as any,
      "circle-stroke-color": "rgba(255,255,255,0.9)",
      "circle-stroke-width": 1,
    },
  ...
  // ── GP surgery search focus (right-click dashed line + ring) ──
  map.addLayer({
    id: "gp-search-focus-line",
    type: "line",
    source: "gp-search-focus",
    filter: ["==", ["geometry-type"], "LineString"] as any,
    paint: {
      "line-color": "#10b981",
      "line-width": 3.5,
  ...
  map.addLayer({
    id: "gp-search-focus-label",
    type: "symbol",
    source: "gp-search-focus",
    filter: ["==", ["geometry-type"], "LineString"] as any,
    layout: {
      "symbol-placement": "line-center",
      "text-field": "GP Surgery",
      "text-size": 12,
      "text-font": ["Open Sans Semibold", "Arial Unicode MS Bold"],
      "text-allow-overlap": true,
      "text-ignore-placement": true,
  ...
  const gpOverlayClickable = () => stateRef.current.gpOverlayMode !== "off";

  const showGpPointPopup = (e: any) => {
    const f = e.features?.[0] as any;
    if (!f) return;
    const p = f.properties || {};
    const name = escapeHtml(String(p.name ?? "GP Surgery"));
    const ods = escapeHtml(String(p.ods_code ?? ""));
    const postcode = escapeHtml(String(p.post_code ?? ""));
    const html = `
      <div style="font:12px/1.5 system-ui,sans-serif;color:#374151;min-width:160px">
  ...
  const showGpClusterPopup = (e: any) => {
    const f = e.features?.[0] as any;
    if (!f) return;
    const count = Number(f.properties?.point_count ?? 0);
    const html = `
      <div style="font-family:system-ui;font-size:12px;line-height:1.25;">
        <div style="font-weight:700;margin-bottom:4px">🏥 GP surgery cluster</div>
        <div>Surgeries in cluster: <b>${count.toLocaleString()}</b></div>
        <div style="opacity:0.8;margin-top:2px">Zoom in to see individual surgeries.</div>
      </div>
    `;
    popup.setLngLat(e.lngLat).setHTML(html).addTo(map);
  };

  map.on("click", "gp-overlay-points",       (e) => { if (!gpOverlayClickable()) return; showGpPointPopup(e); });
  map.on("click", "gp-overlay-clusters",     (e) => { if (!gpOverlayClickable()) return; showGpClusterPopup(e); });
  map.on("click", "gp-overlay-cluster-count",(e) => { if (!gpOverlayClickable()) return; showGpClusterPopup(e); });
  ...
  const overlayHoverLayers = [
    ...
    "gp-overlay-points", "gp-overlay-clusters", "gp-overlay-cluster-count",
  ...
  map.on("mousemove", "cells-fill", (e) => {
    ...
    if (indexPrefsRef.current) {
      const prefs = indexPrefsRef.current;
      const totalPrefWeight = (prefs.affordWeight ?? 0) + (prefs.floodWeight ?? 0) + (prefs.schoolWeight ?? 0) + (prefs.primarySchoolWeight ?? 0) + (prefs.trainWeight ?? 0) + (prefs.ageWeight ?? 0) + (prefs.crimeWeight ?? 0) + (prefs.epcFuelWeight ?? 0) + (prefs.broadbandWeight ?? 0) + (prefs.busWeight ?? 0) + (prefs.pharmacyWeight ?? 0) + (prefs.gpWeight ?? 0) + (prefs.pubWeight ?? 0) + (prefs.supermarketWeight ?? 0);
      ...
      if (Number.isFinite(totalScore)) {
        ...
        if ((prefs.gpWeight ?? 0) > 0) html += wRow("🏥 GP surgery *", prefs.gpWeight!, bar(Number(p.ix_gp ?? 0.5), p.ix_gpn === 1));
  ...
  const clearRgOverlay = () => {
    ...
    setGpSearchFocus(map, null, null);
  ...
  const doReverseGeocode = (lng: number, lat: number) => {
    ...
    const ensureGp = async () => {
      if (_indexGpCache === null) {
        try {
          const r = await fetch("/api/gp-surgeries?plain=1");
          if (r.ok) {
            _indexGpCache = ((await r.json() as any)?.features ?? [])
              .filter((f: any) => f?.geometry?.type === "Point")
              .map((f: any) => ({ lon: Number(f.geometry.coordinates[0]), lat: Number(f.geometry.coordinates[1]), name: String(f.properties?.name ?? ""), ods_code: String(f.properties?.ods_code ?? "") }));
            _indexGpGrid = null;
          }
      ...
      if (_indexGpGrid === null && _indexGpCache !== null) _indexGpGrid = buildSpatialGrid(_indexGpCache, 0.12);
      return _indexGpGrid;
    };
    ...
    void (async () => {
      try {
        ...
        const [floodG, schoolG, primarySchoolG, stationG, crimeG, busStopG, metroTramG, pharmacyG, gpG, pubG, supermarketG, pcRes] = await Promise.all([
          ensureFlood(),
          ensureSchool(),
          ensurePrimarySchool(),
          ensureStation(),
        ...
        // ── Nearest GP surgery ──
        let gpHtml = '<span style="color:#6b7280">No GP surgery data</span>';
        if (gpG) {
          const gpNear = querySpatialGrid(gpG, lng, lat, 0.09); // ~10km radius
          let nearGp: (typeof gpNear)[0] | null = null; let nearGpDist = Infinity;
          for (const sp of gpNear) {
            const d = haversineDistanceMeters(lat, lng, sp.lat, sp.lon);
            if (d < nearGpDist) { nearGpDist = d; nearGp = sp; }
          }
          if (nearGp) {
            const gpCol = nearGpDist <= GP_GREAT_METERS ? "#16a34a" : nearGpDist <= GP_MAX_METERS ? "#d97706" : "#9ca3af";
            gpHtml = `🏥 <span style="color:${gpCol}">${escapeHtml(nearGp.name)}</span> <span style="color:#9ca3af">${fmtDist(nearGpDist)}</span>`;
            lineTargets.push([nearGp.lon, nearGp.lat]);
            setGpSearchFocus(map, { lon: nearGp.lon, lat: nearGp.lat, name: nearGp.name }, { lon: lng, lat });
          } else {
            gpHtml = '<span style="color:#9ca3af">No GP surgery within 6mi</span>';
          }
        }
        ...
        onRightClickInfoRef.current?.({
          stage: 'ready',
          postcode, isOutcode,
          floodHtml, schoolHtml, primarySchoolHtml, stationHtml, crimeHtml, epcHtml, broadbandHtml, busStopHtml, pharmacyHtml, gpHtml, pubHtml, supermarketHtml,
  ...
  useEffect(() => {
    ...
    const gpMode = state.gpOverlayMode ?? "off";
    ...
    const gpVisibility = gpMode === "off" ? "none" : "visible";
    ...
    const hideCellsMode = floodMode === "on_hide_cells" || schoolMode === "on_hide_cells" || primarySchoolMode === "on_hide_cells" || stationMode === "on_hide_cells" || crimeMode === "on_hide_cells" || busStopMode === "on_hide_cells" || pharmacyMode === "on_hide_cells" || gpMode === "on_hide_cells" || pubMode === "on_hide_cells" || supermarketMode === "on_hide_cells" || listedBuildingMode === "on_hide_cells" || planningMode === "on_hide_cells" || holidayLetMode === "on_hide_cells";
    ...
    const apply = () => {
      try {
        ...
        // GP surgery layer visibility
        if (map.getLayer("gp-overlay-points")) map.setLayoutProperty("gp-overlay-points", "visibility", gpVisibility);
        if (map.getLayer("gp-overlay-clusters")) map.setLayoutProperty("gp-overlay-clusters", "visibility", gpVisibility);
        if (map.getLayer("gp-overlay-cluster-count")) map.setLayoutProperty("gp-overlay-cluster-count", "visibility", gpVisibility);
  ...
  }, [state.floodOverlayMode, state.schoolOverlayMode, state.primarySchoolOverlayMode, state.stationOverlayMode, state.crimeOverlayMode, state.busStopOverlayMode, state.pharmacyOverlayMode, state.gpOverlayMode, state.pubOverlayMode, state.supermarketOverlayMode, state.listedBuildingOverlayMode, state.planningOverlayMode, state.holidayLetOverlayMode]);
...
let _indexGpCache: Array<{ lon: number; lat: number; name: string; ods_code: string }> | null = null;
let _indexGpGrid: SpatialGrid<{ lon: number; lat: number; name: string; ods_code: string }> | null = null;
...
async function applyIndexScoring(
  ...
  if (gpW > 0 && (_indexGpCache === null || (_indexGpCache.length === 0 && _indexGpGrid == null))) {
    ...
    loaders.push((async () => {
      try {
        ...
        if (res.ok) {
          const payload = (await res.json()) as any;
          _indexGpCache = (Array.isArray(payload?.features) ? payload.features : [])
            .filter((f: any) => f?.geometry?.type === "Point")
            .map((f: any) => ({ lon: Number(f.geometry.coordinates[0]), lat: Number(f.geometry.coordinates[1]), name: String(f.properties?.name ?? ""), ods_code: String(f.properties?.ods_code ?? "") }));
          _indexGpGrid = null;
        } else { _indexGpCache = []; }
  ...
  if (_indexGpGrid === null && _indexGpCache !== null) _indexGpGrid = buildSpatialGrid(_indexGpCache, GRID_CELL);
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    // 11a) GP surgery proximity
    let gpScore = 0.5;
    let gpNoData = false;
    if (gpW > 0) {
      const wideGp = _indexGpGrid ? querySpatialGrid(_indexGpGrid, cLon, cLat, DATA_DEG) : [];
      if (wideGp.length === 0) {
        gpNoData = true;
      } else if (_indexGpGrid) {
        let minGpDist = Infinity;
        for (const sp of querySpatialGrid(_indexGpGrid, cLon, cLat, 0.09)) {
          const d = haversineDistanceMeters(cLat, cLon, sp.lat, sp.lon);
          if (d < minGpDist) minGpDist = d;
        }
        if (minGpDist <= GP_GREAT_METERS) {
          gpScore = 1.0;
        } else if (minGpDist <= GP_MAX_METERS) {
          gpScore = 1 - (minGpDist - GP_GREAT_METERS) / (GP_MAX_METERS - GP_GREAT_METERS);
        } else {
          gpScore = 0.0;
        }
        totalScore += gpW * gpScore;
        totalWeight += gpW;
      }
    }
    props.ix_gp = gpScore;
    props.ix_gpn = gpNoData ? 1 : 0;

    // 11b) Pub/bar score = 60% average-distance score + 40% count score.
...
function setGpSearchFocus(
  map: maplibregl.Map,
  nearest: { lon: number; lat: number; name: string } | null,
  requested: { lon: number; lat: number } | null
) {
  const source = map.getSource("gp-search-focus") as maplibregl.GeoJSONSource | undefined;
  if (!source) return;
  ...
  if (nearest) {
    ...
    if (requested) {
      features.push({
        type: "Feature",
        properties: { role: "link", label: "GP Surgery" },
        geometry: {
          type: "LineString",
          coordinates: [[requested.lon, requested.lat], [nearest.lon, nearest.lat]],
        },
